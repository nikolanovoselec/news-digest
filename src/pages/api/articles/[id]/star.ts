...
/** ULID format: 26 chars in Crockford base32 alphabet (0-9, A-H, J, K, M,
 *  N, P-T, V-Z — I, L, O, U excluded). All `articles.id` values are ULIDs
 *  generated by the chunk consumer (`generateUlid()`); CF-012 hardens
 *  this endpoint to refuse non-ULID strings up front so `article_stars`
 *  can never be polluted with phantom rows. */
const ULID_REGEX = /^[0-9A-HJKMNP-TV-Z]{26}$/;

/** Extract and validate the article id path parameter. Returns the id
 *  on success or `null` when the value is missing, empty, or not a
...
export async function POST(context: APIContext): Promise<Response> {
  ...
  // CF-012 — refuse to insert a star row for an article that does not
  // exist. Without this check, the ULID regex above would still accept
  // any well-formed-but-unknown id and `INSERT OR IGNORE` would silently
  // create a phantom row in `article_stars`. The lookup is one indexed
  // primary-key SELECT and runs before the optimistic flip lands on the
  // server (the client UI has already optimistically toggled).
  try {
    const exists = await env.DB
      .prepare('SELECT 1 FROM articles WHERE id = ?1')
      .bind(auth.articleId)
      .first();
    if (exists === null) {
    ...
    log('error', 'article.star.failed', {
      ...
      verb: 'POST',
      detail: `existence_check: ${String(err).slice(0, 480)}`,
    });
