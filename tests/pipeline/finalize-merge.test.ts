// Tests for src/lib/finalize-merge.ts — REQ-PIPE-003.
//
...
// idempotency invariant in REQ-PIPE-003 AC 5 — and the winner-
...
describe('pickWinner — REQ-PIPE-003', () => {
  it('REQ-PIPE-003: pickWinner returns the row with the earliest published_at', () => {
  ...
  it('REQ-PIPE-003: pickWinner ties broken by ascending id (deterministic)', () => {
  ...
  it('REQ-PIPE-003: pickWinner of a singleton returns that singleton', () => {
    const winner = pickWinner([row({ id: 'only' })]);
    expect(winner.id).toBe('only');
  });

  it('REQ-PIPE-003: pickWinner throws on empty input', () => {
    expect(() => pickWinner([])).toThrow();
  });
});

describe('buildMergeStatements — REQ-PIPE-003', () => {
  it('REQ-PIPE-003: emits exactly six statements in the documented order', () => {
  ...
  it('REQ-PIPE-003: stars are re-pointed BEFORE the DELETE that would cascade them', () => {
  ...
  it('REQ-PIPE-003: reads are re-pointed BEFORE the DELETE that would cascade them', () => {
    const { db, calls } = makeRecordingDb();
    buildMergeStatements(db, 'winner-1', 'loser-1');
  ...
  it('REQ-PIPE-003: every statement binds winnerId at ?1 and loserId at ?2', () => {
  ...
  it('REQ-PIPE-003: every INSERT uses OR IGNORE so a partial prior attempt does not error on retry', () => {
    const { db, calls } = makeRecordingDb();
    buildMergeStatements(db, 'winner-1', 'loser-1');
    for (let i = 0; i < 5; i++) {
