...
describe('canonicalize', () => {
  ...
  describe('tracking parameter stripping', () => {
    ...
    it('REQ-PIPE-017: strips mc_cid, mc_eid, igshid, si, source', () => {
      expect(canonicalize('https://example.com/a?mc_cid=1&mc_eid=2&igshid=3&si=4&source=rss')).toBe(
        'https://example.com/a'
      );
    });

    it('REQ-PIPE-017: preserves non-tracking params', () => {
    ...
    it('REQ-PIPE-017: strips tracking while keeping real params', () => {
  ...
  describe('trailing slash handling', () => {
    it('REQ-PIPE-017: drops trailing slash from non-root pathname', () => {
      expect(canonicalize('https://example.com/path/')).toBe('https://example.com/path');
      expect(canonicalize('https://example.com/a/b/c/')).toBe('https://example.com/a/b/c');
    });

    it('REQ-PIPE-017: preserves root slash', () => {
    ...
    it('REQ-PIPE-017: drops trailing slash when combined with query string', () => {
      expect(canonicalize('https://example.com/path/?id=1')).toBe('https://example.com/path?id=1');
    });
  });

  describe('malformed input', () => {
    it('REQ-PIPE-017: returns input unchanged when URL is unparseable', () => {
  ...
  describe('combined transformations', () => {
    it('REQ-PIPE-017: applies upgrade + strip + lowercase + trailing-slash in one call', () => {
    ...
    it('REQ-PIPE-017: same logical URL canonicalizes identically from different variants', () => {
