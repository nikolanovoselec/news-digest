...
| `/api/admin/pipeline-run` | Kicker for the backend-driven full pipeline run. POST accepts `{"mode":"full"\|"wipe"}` JSON body and returns `{pipeline_run_id, mode, current_phase, started_at}` (202). GET accepts `?mode=full\|wipe` query string and responds with a `303` redirect to `/settings?pipeline=enqueued&pipeline_run_id=...`; used by the browser settings UI via `window.location.assign()` because CF Access cannot be followed by `fetch()` in CORS mode ([AD38](decisions/README.md#ad38-cf-access-protected-admin-endpoints-must-be-invoked-via-top-level-navigation-not-fetch)). Implements [REQ-OPS-008](../sdd/observability.md#req-ops-008-unified-admin-pipeline-run-from-the-settings-surface). See [API Reference](api-reference.md#post-apiadminpipeline-run). |
