...
| `IS_PRODUCTION` | `"true"` (prod), `"false"` (integration) | Fail-closed gate for dev endpoints and JWKS verification. When `"true"`: routes under `/api/dev/*` return `404` regardless of `DEV_BYPASS_TOKEN` (AC 10); Google `id_token` claims are discarded if JWKS verification fails (unbound KV or unreachable endpoint), forcing fallback to the userinfo endpoint. Missing or unrecognised value is treated as `"true"` (production). Set `"false"` on integration (`news.novoselec.ch`) and local deployments. ([REQ-AUTH-001](../sdd/authentication.md#req-auth-001-sign-in-with-a-federated-identity-provider) AC 10) |
