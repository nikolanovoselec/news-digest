...
2. **Bind Access on the `*.workers.dev` URL too OR disable that subdomain** — Workers & Pages → your worker → Settings → Domains & Routes. Unbound `*.workers.dev` bypasses the Access perimeter; the session + `ADMIN_EMAIL` baseline still gates, but the perimeter promise is broken (AD30).
