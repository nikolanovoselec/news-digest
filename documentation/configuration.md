...
The 0.88 default is the post-2026-05-08 calibration ([AD39](decisions/README.md#ad39-raise-dedup-auto-merge-threshold-to-088-and-gate-merges-to-a-72h-news-cycle-window)). The prior 0.78 missed the dense-theme failure mode: independent articles on a broad topic routinely cosine-match in the 0.78-0.86 band on topical overlap alone, producing a 13-source false-merge cluster spanning 9 days. 0.88 pushes auto-merge above that band; the 0.70-0.88 stripe goes to LLM rerank. The 72h time window keeps cross-news-cycle pairs distinct regardless of cosine. Re-validate both constants if the embedding model or corpus changes substantially. To tune the rerank band, adjust `DEDUP_RERANK_FLOOR`; setting it equal to `DEDUP_COSINE_THRESHOLD` disables rerank without removing the code path.
