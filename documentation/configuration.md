...
| `IS_PRODUCTION` | `"true"` (prod), `"false"` (integration) | Fail-closed gate for dev endpoints. When `"true"`, routes under `/api/dev/*` return `404` regardless of whether `DEV_BYPASS_TOKEN` is set. Missing or unrecognised value is treated as `"true"` (production). Set `"false"` on integration (`news.novoselec.ch`) and local deployments. ([REQ-AUTH-001](../sdd/authentication.md#req-auth-001-sign-in-with-a-federated-identity-provider) AC 10) |
