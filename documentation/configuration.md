...
| `ratelimit:{routeClass}:{identity}:{windowIndex}` | Integer string (request count) | TTL = window size in seconds | Application-layer rate-limit counters written and read by `src/lib/rate-limit.ts`. `routeClass` is one of `auth_login` (10/60 s), `auth_callback` (20/60 s), `auth_refresh` (10/60 s/IP), `auth_logout` (5/60 s/IP), `article_star` (60/60 s), `tags_mutation` (30/60 s). `identity` is the client IP for unauthenticated and IP-keyed routes and the user id for authenticated mutation routes. `windowIndex` is `Math.floor(Date.now() / 1000 / windowSeconds)`. Implements [REQ-AUTH-001](../sdd/authentication.md#req-auth-001-sign-in-with-a-federated-identity-provider) AC 9, [REQ-AUTH-002](../sdd/authentication.md#req-auth-002-access-token--refresh-token-instant-revocation) AC 5. |
