...
The 0.78 default is the recalibrated value from the 2026-05-07 production audit ([AD36](decisions/README.md#ad36-lower-dedup-auto-merge-threshold-to-078-and-remove-the-per-batch-rerank-cap)) — the prior 0.85 was tuned against a single Anthropic financial-AI cluster (pairwise 0.81-0.91) that turned out to be an outlier; real same-news-cycle clusters at production scale span 0.73-0.80 (PAN-OS zero-day cluster 0.75-0.78, Palo Alto Networks valuation week 0.73-0.80). 0.78 captures the upper half of those clusters; the 0.70 rerank floor sends the lower tail to the LLM for a binary same-cycle check. If the embedding model changes or the corpus shifts substantially, re-validate against a fresh sample before relying on these numbers. To tune the rerank band, lower `DEDUP_RERANK_FLOOR` to widen it or raise it to narrow it; setting it equal to `DEDUP_COSINE_THRESHOLD` disables rerank without removing the code path.
