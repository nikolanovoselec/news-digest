...
| Path | Responsibility | Implements |
|---|---|---|
| `src/lib/oauth-providers.ts` | Provider registry: `PROVIDERS` (github + google), per-provider `fetchProfile` adapter, `configuredProviders(env)` returns providers with creds set sorted alphabetically, `userIdFor(provider, sub)` namespaces non-GitHub ids as `<provider>:<sub>` while GitHub keeps its bare-numeric form for legacy compatibility | [REQ-AUTH-001](../sdd/authentication.md#req-auth-001-sign-in-with-a-federated-identity-provider) |
