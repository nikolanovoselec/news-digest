...
| `src/queue/dedup-sweep-consumer.ts` | Queue-driven historical-dedup sweep. Each message runs one batch, re-enqueues a continuation, and CAS-guards `dedup_runs` counters against redelivery. Flips status to `'done'`/`'failed'` at the terminal step. Full Vectorize outage stalls rather than advances the cursor — see AC 18. | [REQ-PIPE-003](../sdd/generation.md#req-pipe-003-same-story-dedupe-across-the-entire-article-history) AC 9, AC 18, [REQ-PIPE-014](../sdd/generation.md#req-pipe-014-same-story-operator-surfaces) |
