...
| `src/lib/discovery.ts` | `discoverTag(tag, env)` — one-shot LLM discovery pipeline with SSRF+parse validation; `processPendingDiscoveries(env, limit)` — cron hook, drains pending rows and writes `sources:{tag}` KV. Uses the shared `extractResponsePayload` helper from `generate.ts` to tolerate both the flat `{ response }` shape (Llama/Mistral) and the OpenAI envelope `{ choices[0].message.content }` shape (gpt-oss-120b); without this, discovery calls on OpenAI-backed models returned `empty_llm_response`. | [REQ-DISC-001](../sdd/discovery.md#req-disc-001-llm-assisted-per-tag-feed-discovery), [REQ-DISC-005](../sdd/discovery.md#req-disc-005-ssrf-protection-for-feed-validation) |
