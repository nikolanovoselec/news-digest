...
| `src/lib/prompts.ts` | `PROCESS_CHUNK_SYSTEM`, `DISCOVERY_SYSTEM`, prompt builders, `LLM_PARAMS`. The `details` field in the LLM output contract is plaintext with paragraphs separated by the JSON-escaped token `\n` (backslash-n); the render side un-escapes it to a real newline. The prompt enforces a 200–250-word HARD CONTRACT per article — a `details` value under 180 words is treated as a failed response. The LLM is explicitly instructed to expand around technical and practical angles rather than shorten output. | [REQ-GEN-005](../sdd/generation.md#req-gen-005-single-call-llm-summarization), [REQ-DISC-001](../sdd/discovery.md#req-disc-001-llm-assisted-per-tag-feed-discovery) |
...
Chunk consumer (per chunk, isolated)
  ...
  → LLM enforces 200–250-word HARD CONTRACT on the details field (≥ 180 words required)
