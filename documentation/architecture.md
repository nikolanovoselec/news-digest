...
| `src/queue/dedup-sweep-consumer.ts` | Operator-triggered historical-dedup sweep consumer. Each message processes one batch via `runHistoricalDedupBatch` then re-enqueues a continuation with the next composite cursor; updates the `dedup_runs` audit row on every step (scanned, merged, last_cursor, remaining) and flips it to `'done'` or `'failed'` at the terminal step. Decouples the sweep from the operator's browser tab. | [REQ-PIPE-003](../sdd/generation.md#req-pipe-003-same-story-dedupe-across-the-entire-article-history) AC 9 |
