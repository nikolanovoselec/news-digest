...
- REQ-PWA-001 AC 6 added: launching the installed PWA from the home-screen icon now holds a brand-canvas splash for ~2.4 seconds before the dashboard appears, so the cold-launch transition reads as a deliberate app boot rather than the previous instant flash. The splash plays once per launched session — subsequent in-app navigations during the same session are unchanged, and users in a regular browser tab never see it.
