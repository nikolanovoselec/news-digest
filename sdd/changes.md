...
- REQ-PIPE-003 AC 7 added: when a single publisher feed surfaces multiple near-duplicate framings of the same event under distinct canonical URLs within the same day (the Google News topical-aggregator case), the duplicates are collapsed into one card before chunk dispatch instead of slipping through as multiple dashboard cards. The collapse never crosses publisher boundaries; cross-publisher duplicates are still handled by the cross-chunk finalize pass.
