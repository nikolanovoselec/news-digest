...
11. Two articles published by the same publisher must clear a stricter same-story bar than two articles from different publishers, so a publisher's recurring writing style does not push unrelated stories from the same outlet over the same-story threshold. The diagnostic surface from AC 10 reports both the raw cosine and the stricter score actually used by the merge decision so an operator can see why a same-publisher pair was kept apart.
