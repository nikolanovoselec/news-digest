...
2. Titles are NYT-style headlines, 45–80 characters, active voice, rewritten rather than copied from the source feed. The 45–80 range is the prompt-side target; the consumer additionally enforces a hard sanity range of 20–200 characters server-side, dropping titles outside that range so genuinely broken cases (single-word labels, paragraph-as-title) never reach the reading surface.
