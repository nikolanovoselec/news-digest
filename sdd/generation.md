...
9. An operator can re-run same-story matching across the entire historical article pool on demand (admin-gated). The sweep runs to completion in the background after the operator triggers it; closing or navigating away from the operator surface does not interrupt the sweep, and progress (articles scanned, duplicates merged, articles remaining) is observable while the sweep is in flight and on the next visit to the operator surface.
