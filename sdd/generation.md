...
15. When a newly-arrived article and an already-stored article describe the same news event, the merge happens regardless of which side was ingested first or which side carries the earlier publication time. A duplicate that lands in a later scrape tick with a publication time predating its already-stored match still collapses to a single card without waiting for an operator-triggered sweep; the survivor is always the article with the earlier publication time (per AC 3) even when that article is the later-arriving one.
