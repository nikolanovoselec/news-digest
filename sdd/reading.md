...
6. The railing's horizontal scroll position is preserved across the cascade — the system does not auto-scroll the railing on tap. On a viewport that scrolls the railing horizontally, the tapped chip slides leftward and may exit off the left edge of the visible area; the user navigates the railing manually to see chips that have moved off-screen. The browser's own implicit scroll triggers on focus / scroll-snap mutation are explicitly defeated so the strip's scrollLeft does not jump.
