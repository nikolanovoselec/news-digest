...
6. The railing's horizontal scroll position is preserved across the cascade — the system does not auto-scroll on tap. On a viewport that scrolls the railing horizontally, the tapped chip slides leftward and may exit off the left edge of the visible area; the user navigates the railing manually to see chips that have moved off-screen. The railing never jumps to a different scroll position as a side effect of the tap — only the user's own swipes change where the railing is scrolled.
