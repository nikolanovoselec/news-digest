# Spec Changes — Archive (April 2026)

Archived from `sdd/changes.md` on 2026-05-03 by `/sdd clean`. Covers the late-April rework window (2026-04-22 through 2026-04-26): the global-feed pipeline replacing per-user digest generation, federated OAuth, the curated-source registry refresh, the access+refresh token model, and the day-by-day polish on tag railing, timezone correction, and email content.

Active changelog continues in `sdd/changes.md` from 2026-04-27 onward.
...
- Initial product specification bootstrapped from `requirements.md` via `/sdd init` with `enforce_tdd: true`. Scope: 10 panels covering authentication, onboarding, source discovery, digest generation, reading, email, history, design system, PWA, and observability.
