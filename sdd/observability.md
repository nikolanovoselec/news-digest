...
3. While either run is in flight both actions are disabled and a status line reports the current phase in user-readable prose ("Re-embedded N articles, M remaining…", "Scrape kicked; waiting for it to finish…", "Embedded N new articles. Running dedup…", "Done — embedded N new, scanned X, merged Y duplicates.", "Scrape finished." for the refresh-only path). The status line updates as each phase advances; both actions re-enable when the run finishes or errors.
